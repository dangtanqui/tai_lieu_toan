...
Nghiệm cuối cùng của Gradient Descent phụ thuộc rất nhiều vào điểm khởi tạo và learning rate. Trong phần này, ta đề cập một vài phương pháp thường được dùng để khắc phục những hạn chế của GD. Đồng thời, các thuật toán biến thể của GD thường được áp dụng trong các mô hình Deep Learning cũng được tổng hợp.
\end{ynghia}
...
\begin{ynghia}[title={Gradient dưới góc nhìn vật lý}]
Thuật toán GD thường được ví với tác dụng của trọng lực lên một hòn bi đặt trên một mặt có dạng như hình một thung lũng giống như hình dưới đây. Bất kể ta đặt hòn bi ở $A$ hay $B$ thì cuối cùng hòn bi cũng sẽ lăn xuống và kết thúc ở vị trí $C$.
\end{ynghia}

\mlcbimg{02_gradient_descent_part2/img03.png}

\begin{luuy}[title={Hai đáy thung lũng}]
Tuy nhiên, nếu bề mặt có hai đáy thung lũng như hình (b) thì tùy vào việc đặt bi ở $A$ hay $B$, vị trí cuối cùng của bi sẽ ở $C$ hoặc $D$. Điểm $D$ là một điểm local minimum chúng ta không mong muốn.
\end{luuy}

\begin{ynghia}[title={Vượt local minimum nhờ đà}]
Nếu suy nghĩ một cách vật lý hơn, vẫn trong hình (b), nếu vận tốc ban đầu của bi khi ở điểm $B$ đủ lớn, khi bi lăn đến điểm $D$, theo đà, bi có thể tiếp tục di chuyển lên dốc phía bên trái của $D$. Và nếu giả sử vận tốc ban đầu lớn hơn nữa, bi có thể vượt dốc tới điểm $E$ rồi lăn xuống $C$ như trong hình (c). Đây chính là điều chúng ta mong muốn. Có thể đặt câu hỏi: liệu bi lăn từ $A$ tới $C$ có theo đà lăn tới $E$ rồi tới $D$ không? Điều này khó xảy ra hơn vì nếu so với dốc $DE$ thì dốc $CE$ cao hơn nhiều.
...
\begin{ynghia}[title={Gradient Descent với Momentum}]
...
\begin{itemize}
 \item Không vượt qua được (Bị kẹt): Nếu hệ số quán tính $\gamma$ quá nhỏ hoặc hố cực tiểu quá sâu và rộng, "viên bi" momentum sẽ không tích lũy đủ động năng để vượt qua dốc và thoát khỏi cực tiểu địa phương (local minimum).
 \item Vượt quá đà (Overshooting): Ngược lại, nếu quán tính quá lớn, nó có thể giúp vượt qua các cực tiểu địa phương nhỏ nhưng lại dễ làm thuật toán "chạy quá" điểm cực tiểu toàn cục (global minimum), gây ra hiện tượng dao động mạnh và khó hội tụ chính xác tại đáy.
\end{itemize}
\end{luuy}
...
\begin{ynghia}[title={Ý tưởng chính: Nhìn trước một bước}]
...
Các hình trên cùng là đường đi của nghiệm với phương pháp Momentum: nghiệm đi khá zigzag và mất nhiều vòng lặp hơn. Các hình dưới đó là đường đi của nghiệm với phương pháp NAG: nghiệm hội tụ nhanh hơn, và đường đi ít zigzag hơn.
\end{vidu}
...
Ngoài Momentum và NAG, trong Deep Learning thường dùng các optimizer \textbf{thích ứng learning rate theo từng tọa độ}: \textbf{Adagrad}, \textbf{RMSprop}, \textbf{Adam} (kết hợp momentum và RMSprop).
\end{ynghia}
...
Hàm mất mát và đạo hàm của nó cho bài toán này lần lượt là (để cho thuận tiện, trong bài này ta dùng ký hiệu $\mathbf{X}$ thay cho dữ liệu mở rộng $\bar{\mathbf{X}}$):
...
\begin{luuy}[title={Thứ tự lựa chọn điểm dữ liệu: Shuffle sau mỗi epoch}]
...
%------------------------------------------------------------------------------
\subsection{Stopping Criteria (điều kiện dừng)}
...
%------------------------------------------------------------------------------
\subsection{Một phương pháp tối ưu đơn giản khác: Newton's method}
\label{subsec:newton}
%------------------------------------------------------------------------------
