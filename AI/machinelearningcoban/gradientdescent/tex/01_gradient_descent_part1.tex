%==============================================================================
\section{Cơ sở Gradient Descent}
\label{sec:gd-part1}
%==============================================================================
...
\begin{itemize}
\item Điểm local minimum $x^*$ của hàm số là điểm có đạo hàm $f'(x^*) =0$. Hơn thế nữa, trong lân cận của nó, đạo hàm của các điểm phía bên trái $x^*$ là không dương, đạo hàm của các điểm phía bên phải $x^*$ là không âm.
\item Đường tiếp tuyến với đồ thị hàm số đó tại một điểm bất kỳ có hệ số góc chính bằng đạo hàm của hàm số tại điểm đó.
\end{itemize}
\end{dinhnghia}
...
%------------------------------------------------------------------------------
\subsection{Gradient Descent cho hàm một biến}
%------------------------------------------------------------------------------
...
Với $f$ khả vi, nếu $f'(x_t) \neq 0$ và $|\eta f'(x_t)|$ đủ nhỏ thì $f(x_{t+1}) < f(x_t)$. Đây là cơ sở lý thuyết cho việc chọn $\eta$ không quá lớn.
\end{ynghia}
...
\begin{vidu}[title={Điểm khởi tạo khác nhau: Thử $x_0 = -5$ và $x_0 = 5$}]
...
$x_0 = -5$:
\mlcbframegrid{4}{01_gradient_descent_part1/img03}{0}{10}

$x_0 = 5$:
\mlcbframegrid{4}{01_gradient_descent_part1/img04}{0}{28}
...
\begin{vidu}[title={Learning rate khác nhau: Cùng $x_0 = -5$, $\eta$ khác nhau}]
...
$\eta = 0.01$:
\mlcbframegrid{5}{01_gradient_descent_part1/img05}{0}{98}

$\eta = 0.5$:
\mlcbframegrid{5}{01_gradient_descent_part1/img06}{0}{98}
\end{vidu}
...
%------------------------------------------------------------------------------
\subsection{Gradient Descent cho hàm nhiều biến}
%------------------------------------------------------------------------------
...
Với MSE và ma trận đầy đủ hạng, nghiệp vụ tối thiểu là duy nhất (hàm lồi). GD và công thức ma trận \textbf{cùng hướng tới cùng $\mathbf{w}^*$} nếu $\eta$ phù hợp. GD vẫn quan trọng vì:
\begin{itemize}
\item Không cần lưu nghịch đảo ma trận lớn.
\item Mở rộng được khi thêm regularization hoặc mô hình phi tuyến.
\item Là mẫu cho SGD khi $N$ rất lớn.
\end{itemize}
\end{dongco}
...
\mlcbimg{01_gradient_descent_part1/img07.png}
...
\begin{dinhnghia}[title={Kiểm tra đạo hàm: Numerical gradient}]
...
\begin{ynghia}[title={Giải thích bằng hình học: Ba vector xấp xỉ so với đạo hàm chính xác}]
...
\begin{phuongphap}[title={Giải thích bằng giải tích: Khai triển Taylor và sai số $O(\varepsilon)$ vs.\ $O(\varepsilon^2)$}]
...
\begin{luuy}[title={Với hàm nhiều biến: Áp dụng numerical gradient cho nhiều biến}]
...
Các đường thẳng màu đỏ là nghiệm tìm được sau mỗi vòng lặp:
\mlcbframegrid{5}{01_gradient_descent_part1/img09}{0}{48}

Đường đồng mức:
\mlcbframegrid{5}{01_gradient_descent_part1/img10}{0}{48}
\end{vidu}
...
Trong bản đồ tự nhiên, để miêu tả độ cao của các dãy núi, người ta dùng nhiều đường cong kín bao quanh nhau.

\mlcbimg{01_gradient_descent_part1/img11.jpg}
...
\mlcbframegrid{5}{01_gradient_descent_part1/img12}{0}{98}
\mlcbframegrid{5}{01_gradient_descent_part1/img13}{0}{98}
\end{vidu}
...
%------------------------------------------------------------------------------
\subsection{Ví dụ: hai cực tiểu phụ thuộc khởi tạo}
%------------------------------------------------------------------------------
...
\mlcbframegrid{5}{01_gradient_descent_part1/img14}{0}{98}
\end{vidu}
