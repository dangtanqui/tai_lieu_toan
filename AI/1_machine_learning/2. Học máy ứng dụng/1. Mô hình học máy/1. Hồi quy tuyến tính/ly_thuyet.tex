...
\begin{vidu}[title={Tính $L_2$ loss cho một điểm}]
...
\begin{ynghia}[title={Outlier trong dữ liệu và trong dự đoán}]
...
\begin{tinhchat}[title={So sánh trực quan Hình 9 và 10}]
\begin{itemize}
...
\begin{luuy}[title={Kiểm tra hiểu biết --- MSE trên hai đồ thị}]
\begin{center}
...
\end{center}
\textbf{Đồ thị bên phải (Hình 12)} có MSE cao hơn ($\approx 0{,}8$ vs $\approx 0{,}4$): nhiều điểm trên đường nhưng hai điểm lệch \textbf{gấp đôi} khoảng cách; bình phương làm loss tăng mạnh ($2^2 = 4$ lần so với lệch 1).
\end{luuy}
...
\begin{vidu}[title={Đáp án tham khảo (Google)}]
...
\begin{ynghia}
Bài tập củng cố: \textbf{tham số} ($w$, $b$) quyết định độ nghiêng và vị trí đường; \textbf{MSE} đo tổng bình phương sai số --- gradient descent sau này tự làm việc này.
\end{ynghia}
...
\begin{enumerate}
  ...
  \item Đạo hàm theo $w$: độ dốc tiếp tuyến $\approx -119{,}7$; theo $b$: $\approx -34{,}3$.
  \item Bước nhỏ $\eta = 0{,}01$ (learning rate):
  \[
    w_{\text{mới}} = 0 - 0{,}01 \times (-119{,}7) \approx 1{,}2,\quad
    b_{\text{mới}} = 0 - 0{,}01 \times (-34{,}3) \approx 0{,}34
  \]
\end{enumerate}
\end{phuongphap}

\begin{congthuc}[title={Đạo hàm MSE (trực giác)}]
\[
...
\begin{dinhnghia}[title={Tốc độ học}]
  Tốc độ học xác định mức độ thay đổi cần thực hiện đối với các trọng số và độ lệch trong mỗi bước của quy trình hạ độ dốc. 
  
  Hệ số $\eta$ nhân với gradient để quyết định \textbf{độ lớn bước} cập nhật $w$, $b$ mỗi vòng.
...
Số \href{https://developers.google.com/machine-learning/glossary\#example}{mẫu} xử lý trước mỗi lần cập nhật $w$, $b$.
\end{dinhnghia}
...
Một mức nhiễu có thể giúp \href{https://developers.google.com/machine-learning/glossary\#generalization}{tổng quát hóa} tốt hơn.
\end{ynghia}

\subsection{Epoch}

\begin{dinhnghia}[title={Epoch}]
Một \href{https://developers.google.com/machine-learning/glossary\#epoch}{epoch} = mô hình đã duyệt \textbf{toàn bộ} tập huấn luyện đúng một lần. Ví dụ: 1000 mẫu, batch 100 $\Rightarrow$ 10 \href{https://developers.google.com/machine-learning/glossary\#iteration}{iteration} / epoch.
\end{dinhnghia}
...
\midrule
Full batch & 20 lần (1 lần / epoch). \\
SGD & 20\,000 lần (1 mẫu / epoch). \\
...
\begin{phuongphap}[title={Ba nhiệm vụ (Gradient descent exercise)}]
\begin{description}
