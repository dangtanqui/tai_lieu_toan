...
Cần nắm \href{https://developers.google.com/machine-learning/intro-to-ml}{Introduction to ML}, \href{https://developers.google.com/machine-learning/crash-course/linear-regression}{Linear regression} và \href{https://developers.google.com/machine-learning/crash-course/logistic-regression}{\textbf{Logistic regression}}.
\end{luuy}
...
Tránh mặc định 0,5 khi: lớp mất cân bằng (0,01\% spam); chi phí FP/FN không đối xứng; sai lọc thư hợp lệ đắt hơn để spam lọt.
\end{luuy}

\subsection{Ma trận nhầm lẫn}

\begin{dinhnghia}[title={Confusion matrix}]
Điểm dự đoán $\neq$ \href{https://developers.google.com/machine-learning/glossary\#ground_truth}{ground truth}. Bốn ô (cột = thực tế, hàng = dự đoán):
\end{dinhnghia}
...
\midrule
\textbf{Dự đoán dương} &
\textbf{TP} --- spam đúng vào thư rác &
\textbf{FP} --- thư hợp lệ vào spam \\
\textbf{Dự đoán âm} &
\textbf{FN} --- spam lọt hộp thư &
\textbf{TN} --- thư hợp lệ đúng hộp thư \\
\end{tabularx}
\end{congthuc}
...
Tổng thực tế dương không gần bằng tổng âm $\Rightarrow$ \href{https://developers.google.com/machine-learning/glossary\#class_imbalanced_data_set}{mất cân bằng lớp}. Ví dụ: hàng nghìn ảnh mây, loại volutus chỉ vài ảnh.
\end{vidu}
...
\begin{luuy}[title={Kiểm tra hiểu biết --- Ngưỡng}]
\textbf{Câu 1.} Site hợp lệ bị gán malware $\Rightarrow$ \textbf{FP} (âm bị gán dương).

\medskip
\textbf{Câu 2.} Tăng ngưỡng $\Rightarrow$ \textbf{cả TP và FP giảm} (ít gán dương; ví dụ ngưỡng 0,9999).

\medskip
\textbf{Câu 3.} Tăng ngưỡng $\Rightarrow$ \textbf{TN tăng, FN giảm} (theo đáp án MLCC; thử trên widget).
\end{luuy}
...
\begin{itemize}
  \item \textbf{Accuracy:} tỷ lệ email phân loại đúng.
  \item \textbf{Recall (TPR):} trong spam thật, bắt được bao nhiêu? (\emph{probability of detection})
  \item \textbf{FPR:} tỷ lệ thư hợp lệ bị gán nhầm spam.
  \item \textbf{Precision:} trong email gán spam, bao nhiêu đúng là spam?
\end{itemize}
\end{ynghia}

\begin{phanvidu}[title={Accuracy trên tập mất cân bằng}]
99\% nhãn âm: mô hình luôn dự đoán âm $\Rightarrow$ accuracy 99\% nhưng \textbf{vô dụng}.
\end{phanvidu}

\begin{tinhchat}[title={Precision vs recall}]
Tăng ngưỡng $\Rightarrow$ thường precision $\uparrow$, recall $\downarrow$ (đánh đổi nghịch).
\end{tinhchat}
...
$TP=5$, $TN=6$, $FP=3$, $FN=2$: Recall $=5/7\approx 0{,}714$; Precision $=5/8=0{,}625$.

\medskip
$TP=3$, $FP=2$, $FN=1$: Precision $=3/5=0{,}6$.
\end{vidu}

\begin{luuy}[title={Kiểm tra hiểu biết --- Metric}]
\textbf{Câu 1.} $TP=5$, $FN=2$ $\Rightarrow$ Recall $=5/7$ (không chia cho tổng mọi phân loại đúng).

\medskip
\textbf{Câu 2.} $TP=3$, $FP=2$ $\Rightarrow$ Precision $=3/5$ (chia cho mọi dự đoán dương).

\medskip
\textbf{Câu 3.} Bẫy côn trùng xâm lấn: FN đắt, FP rẻ $\Rightarrow$ tối đa \textbf{Recall}.
\end{luuy}
...
\begin{luuy}[title={Kiểm tra hiểu biết --- ROC}]
\textbf{Câu 1.} AUC cao nhất: \path{auc_0-77.png} ($\approx 0{,}77$).

\medskip
\textbf{Câu 2.} Tệ hơn ngẫu nhiên: \path{auc_0-32.png} (AUC nhỏ hơn 0,5).

\medskip
\textbf{Câu 3.} Cải thiện không huấn luyện lại: \textbf{đảo nhãn dự đoán}.

\medskip
\textbf{Câu 4.} Thư quan trọng không được vào spam $\Rightarrow$ \textbf{điểm A} (FPR thấp).
\end{luuy}
...
Một mẫu thuộc \textbf{nhiều lớp cùng lúc} $\Rightarrow$ \textbf{đa nhãn} (multi-label), khác đa lớp. Chi tiết: \href{https://developers.google.com/machine-learning/crash-course/neural-networks/multi-class}{Multi-class neural networks}.
\end{luuy}
