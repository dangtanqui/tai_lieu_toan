...
\begin{ynghia}[title={Bối cảnh và đặc điểm}]
...
Critic cập nhật theo \textbf{Temporal Difference}, thường dạng $TD(0)$. Mục tiêu xấp xỉ Bellman với policy \textbf{target} $\mu_{\theta'}$:
\[
...
DDPG duy trì bản sao \textbf{chậm} của actor và critic: $\mu_{\theta'}$, $Q_{\phi'}$. Target dùng trong $y$ ở bước critic để tránh ``đuổi theo'' mục tiêu thay đổi quá nhanh --- nguyên nhân hay gặp của \textbf{dao động} khi dùng xấp xỉ hàm sâu (tương tự DQN).
\end{ynghia}
