...
\begin{itemize}
  \item \textbf{Bước E (Expectation)}: gán điểm vào cụm gần nhất.
  \item \textbf{Bước M (Maximization)}: tính lại centroid mỗi cụm.
\end{itemize}
\end{dinhnghia}
...
\begin{itemize}
  ...
  \item \textbf{Cluster-then-predict}: xây mô hình riêng cho từng nhóm con sau khi phân cụm.
\end{itemize}
\end{dongco}
...
\begin{vidu}[title={Mã đầy đủ (ví dụ 1)}]
\begin{lstlisting}
import numpy as np
import matplotlib.pyplot as plt
from sklearn.cluster import KMeans
...
\begin{itemize}
  ...
  \item \textbf{Nhạy rescale}: chuẩn hóa/chuẩn tắc hóa làm thay đổi hoàn toàn phân cụm --- cần xử lý nhất quán.
...
\begin{ynghia}[title={Phân đoạn ảnh}]
Gom vùng pixel theo màu hoặc texture; dùng trong nhận dạng đối tượng, truy vấn ảnh, ảnh y khoa.
\end{ynghia}

\begin{ynghia}[title={Phân khúc khách hàng}]
Chia khách theo hành vi mua hoặc nhân khẩu; hỗ trợ giữ chân khách, loyalty, quảng cáo nhắm mục tiêu.
\end{ynghia}

\begin{ynghia}[title={Phát hiện bất thường}]
Điểm không thuộc cụm nào (hoặc xa mọi centroid) có thể là anomaly --- gian lận, xâm nhập mạng, bảo trì dự đoán.
\end{ynghia}

\begin{ynghia}[title={Phân tích biểu hiện gen}]
Gom gen đồng điều hòa/biểu hiện; hỗ trợ khám phá thuốc, chẩn đoán, y học cá thể hóa.
\end{ynghia}
