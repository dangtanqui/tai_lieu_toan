...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_skills.htm}.
\end{ynghia}
...
Nguồn khuyến nghị: nếu muốn theo học máy như một hướng nghề nghiệp, hãy học đủ các kỹ năng giúp tăng năng lực tổng thể.
\end{dongco}

\subsection{Danh sách kỹ năng chính (theo nguồn)}

\begin{phuongphap}[title={Key skills}]
\begin{itemize}
  \item Programming Skills
  \item Statistics and Mathematics
...
\begin{luuy}[title={Hình minh họa trong nguồn}]
...
\begin{phuongphap}[title={Python và R (theo nguồn)}]
\begin{itemize}
  ...
  \item Các nội dung Python cơ bản nguồn gợi ý: basic data types; dictionaries/lists/sets; loops/conditionals; functions; list comprehensions.
  \item R cũng phổ biến trong ML; nguồn nói có thể làm các tác vụ ML nặng “dễ hơn”; ngoài ngôn ngữ, cần nắm các package đi kèm.
\end{itemize}
\end{phuongphap}
...
\begin{ynghia}[title={Thống kê dùng để làm gì? (theo nguồn)}]
...
Nguồn chia thống kê thành:
\begin{itemize}
  \item \textbf{Descriptive statistics}: đơn giản hoá/tổ chức dữ liệu bằng mean, range, variance, standard deviation.
  \item \textbf{Inferential statistics}: dùng mẫu nhỏ để kết luận về tập lớn, qua hypothesis testing, null/alternative testing, \ldots
\end{itemize}
\end{ynghia}

\subsubsection{Mathematics}
\begin{phuongphap}[title={Các mảng toán nên biết (theo nguồn)}]
\begin{itemize}
  \item \textbf{Algebra}: biến, hằng, hàm; phương trình tuyến tính; logarit.
  \item \textbf{Linear algebra}: vectơ, ma trận, eigenvalues.
  \item \textbf{Calculus}: đạo hàm, tích phân, gradient descent.
\end{itemize}
\end{phuongphap}

\begin{vidu}[title={Toán liên quan ML như thế nào? (theo nguồn)}]
...
Nguồn nhấn mạnh: mức toán cần biết có thể chỉ ở mức beginner, nhưng điều quan trọng là bạn đọc và hiểu ký hiệu toán trong phương trình.
...
Nguồn gợi ý nắm: biến ngẫu nhiên, mật độ/phân phối xác suất, \ldots
\end{ynghia}

\begin{vidu}[title={Ví dụ kiểm tra kiến thức: phân loại theo xác suất có điều kiện (theo nguồn)}]
\[
...
Nguồn đưa ví dụ một bài toán tối ưu (mang tính kiểm tra khả năng đọc ký hiệu).
...
Nguồn nêu một số cấu trúc: arrays, stack, queue, binary trees, maps, \ldots
\end{ynghia}
...
Nguồn nêu công cụ: Tableau, Power BI, \ldots
\end{ynghia}
...
Nguồn liệt kê: regression, decision trees, random forests, k-NN, SVM, neural networks.
...
Nguồn nêu kỹ thuật như sentiment analysis, text classification, named entity recognition.
\end{ynghia}
...
% The downloader currently stores this page's images under the older combined slug.
\InputIfFileExists{_figures/04_yeu_cau_nen_tang_va_ky_nang.tex}{}{}
