...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_basics.htm}.
\end{ynghia}
...
Data, feature, model, training, testing: Mục~\ref{sec:thanh-phan-co-lo} và~\ref{sec:train-val-test}. Bài này giữ phần \textbf{Tom Mitchell (T--P--E)} và hướng dẫn giảm overfitting/underfitting theo nguồn.
\end{luuy}

\subsection{Các thành phần cốt lõi (tóm tắt)}

\begin{ynghia}[title={Data, Feature, Model, Training, Testing}]
Xem định nghĩa đầy đủ ở Mục~\ref{sec:thanh-phan-co-lo} và~\ref{sec:train-val-test}. Nguồn nhấn mạnh: mô hình được đánh giá trên tập validation/test riêng để kiểm tra khả năng tổng quát.
\end{ynghia}

\subsection{Overfitting (Quá khớp)}
...
\begin{phuongphap}[title={Cách giảm overfitting (theo nguồn)}]
\begin{itemize}
...
\subsection{Underfitting (Thiếu khớp)}
...
\begin{phuongphap}[title={Cách giảm underfitting (theo nguồn)}]
\begin{itemize}
...
\begin{phuongphap}[title={Một số tình huống (theo nguồn)}]
\begin{itemize}
...
\begin{phuongphap}[title={Cách diễn giải ngắn (theo nguồn)}]
...
Nguồn nêu một số metric: accuracy score, F1 score, confusion matrix, precision, recall, sensitivity, \ldots
\end{ynghia}
