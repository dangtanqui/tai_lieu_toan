...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_data_structure.htm}.
\end{ynghia}
...
\begin{ynghia}[title={Mục tiêu của bài}]
...
\begin{phuongphap}[title={Một số tác vụ dùng array (theo nguồn)}]
\begin{itemize}
  \item Dữ liệu thô thường biểu diễn dưới dạng array.
  \item Chuyển pandas DataFrame sang list (nguồn nêu lý do: pandas series yêu cầu cùng kiểu; Python list có thể chứa nhiều kiểu).
...
Nguồn nêu: queue dùng trong multi-threading để tối ưu và điều phối luồng dữ liệu giữa các thread; thường dùng để xử lý dữ liệu lớn và feed batch cho training để train liên tục và hiệu quả.
\end{dinhnghia}

\subsubsection{6) Trees}
...
\begin{ynghia}[title={Ứng dụng graph (theo nguồn)}]
Graph dùng nhiều trong recommendation systems, link prediction và phân tích mạng xã hội.
\end{ynghia}

\subsubsection{8) Hash Maps}
\begin{dinhnghia}[title={Hash maps}]
Nguồn nêu hash maps thường dùng trong ML nhờ khả năng lưu và truy xuất key--value.
...
Nguồn lưu ý: ngoài các cấu trúc trên, nhiều thư viện/framework ML còn cung cấp cấu trúc chuyên biệt cho use case cụ thể, như matrices và tensors cho deep learning.
...
Nguồn mô tả: cây được tạo bằng cách chia dữ liệu đệ quy theo feature “thông tin nhất”; cấu trúc tree giúp dễ duyệt quá trình ra quyết định và dự đoán.
\end{phuongphap}
