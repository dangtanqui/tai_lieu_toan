...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_introduction.htm}.
\end{ynghia}
...
Định nghĩa ML và bốn kiểu học xem Mục~\ref{sec:khai-niem-nen}. Bài này bổ sung lịch sử, cost function, so sánh ML/traditional programming, đạo đức.
\end{luuy}

\subsection{Học máy là gì?}

\begin{ynghia}[title={Định nghĩa}]
Xem Mục~\ref{sec:ml-la-gi}. Nguồn nhấn mạnh \textbf{model building}: huấn luyện thuật toán trên dữ liệu để học hidden patterns.
\end{ynghia}

\subsection{Học máy hoạt động như thế nào?}

\begin{phuongphap}[title={3 thành phần chính (theo nguồn)}]
...
\begin{phuongphap}[title={Các mốc (theo nguồn)}]
\begin{itemize}
...
\subsection{Các phương pháp học máy}

\begin{ynghia}[title={Bốn nhóm chính}]
Supervised, unsupervised, semi-supervised, reinforcement --- định nghĩa chi tiết ở Mục~\ref{sec:bon-kieu-hoc}. So sánh supervised vs unsupervised: Mục~\ref{sec:supervised-vs-unsupervised}.
\end{ynghia}

\subsection{Use cases của học máy}

\begin{phuongphap}[title={Một số use case (theo nguồn)}]
\begin{itemize}
  ...
  \item \textbf{Drive-less cars}: xe tự lái; ví dụ Tesla (theo nguồn).
...
\begin{phuongphap}[title={Advantages of Machine Learning (theo nguồn)}]
\begin{itemize}
...
\begin{phuongphap}[title={Disadvantages of Machine Learning (theo nguồn)}]
\begin{itemize}
...
\begin{phuongphap}[title={Challenges in Machine Learning (theo nguồn)}]
\begin{itemize}
...
\begin{vidu}[title={So sánh theo tiêu chí (dịch từ bảng trong nguồn)}]
...
\begin{ynghia}[title={ML vs Deep Learning (theo nguồn)}]
Deep learning là một nhánh của ML, khác nhau ở cách thuật toán học: deep learning dùng cấu trúc “giống não người” (mạng sâu).
Nguồn nêu deep learning thường hiệu quả hơn ở bài toán phức tạp (ví dụ xe tự lái).
\end{ynghia}

\begin{ynghia}[title={ML vs Generative AI (theo nguồn)}]
...
\begin{phuongphap}[title={Một số xu hướng (theo nguồn)}]
...
\begin{phuongphap}[title={Lộ trình 5 bước (theo nguồn)}]
\begin{enumerate}
