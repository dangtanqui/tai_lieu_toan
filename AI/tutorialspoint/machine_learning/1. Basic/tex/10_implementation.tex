...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_implementing.htm}.
\end{ynghia}
...
\subsection{Các bước triển khai học máy (theo nguồn)}
...
Sau khi thu thập, cần tiền xử lý và làm sạch để loại bỏ bất nhất hoặc thiếu dữ liệu.
\end{phuongphap}
...
Nguồn nêu các công cụ như Matplotlib và Seaborn để vẽ histogram, scatter plots, heatmaps.
\end{phuongphap}
...
Nguồn nhắc: chia train/test và fit mô hình trên train.
...
Nguồn nêu metric như accuracy, precision, recall, F1-score; và có thể dùng cross-validation để kiểm thử khả năng tổng quát.
\end{phuongphap}

\subsubsection{Model Tuning}
\begin{phuongphap}[title={Tuning siêu tham số}]
Có thể cải thiện hiệu năng bằng tuning hyperparameters (các tham số không học từ dữ liệu mà do người dùng đặt).
Nguồn nêu: tìm bộ tối ưu bằng grid search và random search.
\end{phuongphap}
...
\begin{phuongphap}[title={Ngôn ngữ hỗ trợ ML (theo nguồn)}]
Python, R, Matlab, Octave, Julia, C++, C.
Nguồn lưu ý: danh sách không đầy đủ, nhưng gồm các ngôn ngữ phổ biến.
\end{phuongphap}

\subsubsection{IDEs}
\begin{phuongphap}[title={IDE hỗ trợ ML (theo nguồn)}]
R Studio, PyCharm, iPython/Jupyter Notebook, Julia, Spyder, Anaconda, Rodeo, Google Colab.
Nguồn lưu ý: danh sách không đầy đủ; mỗi IDE có ưu/nhược; nên thử trước khi chọn một IDE chính.
\end{phuongphap}
