...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_real_life_examples.htm}.
\end{ynghia}
...
Nguồn đưa ví dụ thực tế như trợ lý ảo và chatbot (Google Assistant, Siri, Alexa), hệ gợi ý, Tesla autopilot, IBM Watson for Oncology, \ldots
\end{dongco}
...
\subsection{Danh sách ví dụ (theo nguồn)}
\begin{itemize}
  \item Virtual Assistants and Chatbots
  \item Fraud Detection in Banking and Finance
  \item Healthcare Diagnosis and Treatment
...
Trợ lý ảo tương tác qua lệnh giọng nói, có thể thay công việc trợ lý cá nhân như gọi điện, lên lịch hẹn, hoặc đọc email to.
Nguồn nêu các trợ lý phổ biến: Alexa, Apple Siri, Google Assistant.
\end{ynghia}
...
\begin{ynghia}[title={Ví dụ thực tế (theo nguồn)}]
...
Nguồn nêu PayPal là một ví dụ dùng ML để cải thiện giao dịch hợp lệ trên nền tảng.
\end{ynghia}
...
\begin{ynghia}[title={Ví dụ (theo nguồn)}]
...
Nguồn nêu IBM Watson for Oncology dùng ML để phân tích hồ sơ y tế và đề xuất điều trị ung thư cá nhân hoá.
\end{ynghia}
...
\begin{ynghia}[title={Ví dụ (theo nguồn)}]
Nguồn nêu xe tự hành có thể giúp giảm tai nạn và tăng hiệu quả.
Một số công ty: Tesla, Waymo, Uber đang dùng ML để phát triển xe tự lái.
Nguồn nhắc Tesla Vision dùng camera/cảm biến và neural net processing để hiểu môi trường; AutoPilot là hệ thống hỗ trợ lái nâng cao.
\end{ynghia}
...
\begin{ynghia}[title={Các ví dụ hệ gợi ý (theo nguồn)}]
\begin{itemize}
...
\begin{dinhnghia}[title={Quảng cáo nhắm mục tiêu}]
...
Nguồn nêu ứng dụng: nhận diện khuôn mặt, tìm kiếm hình ảnh (visual search), chẩn đoán y khoa, nhận dạng người, \ldots
\end{dinhnghia}
