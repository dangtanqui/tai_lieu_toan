...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_models.htm}.
\end{ynghia}
...
Bài này giới thiệu các \textbf{nhóm phương pháp} chính; chi tiết từng nhóm ở các bài 25--28.
\end{dongco}
...
\begin{luuy}[title={Đối chiếu Khái niệm nền}]
Định nghĩa tổng quan về bốn kiểu học: Mục~\ref{sec:bon-kieu-hoc}; bài này bổ sung \textbf{phân nhánh tác vụ} và danh sách mô hình thường gặp.
\end{luuy}
...
Mục tiêu: học ánh xạ từ đầu vào $x$ sang đầu ra $Y$ sau nhiều lần huấn luyện:
\[
Y = f(x).
\]
...
Nguồn liệt kê KNN trong ngữ cảnh unsupervised --- thực tế \textbf{k-NN chủ yếu là thuật toán supervised} (classification/regression). Clustering thường dùng K-Means, DBSCAN, \ldots
\end{luuy}
...
\begin{phuongphap}[title={Hai hướng triển khai (theo nguồn)}]
\begin{enumerate}
...
\begin{phuongphap}[title={Sáu bước (theo nguồn)}]
\begin{enumerate}
...
\begin{luuy}[title={Tiếp theo}]
Chi tiết từng kiểu học: Mục~\ref{sec:supervised-vs-unsupervised}, \ref{sec:supervised-learning}, \ref{sec:unsupervised-learning}, \ref{sec:reinforcement-learning}.
\end{luuy}
