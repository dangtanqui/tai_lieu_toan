...
\begin{vidu}[title={Hình minh hoạ hiểu dữ liệu}]
\tsimg{10_du_lieu_trong_ml/img01.jpg}
\end{vidu}

\subsection{Các bước trong giai đoạn hiểu dữ liệu}

\begin{phuongphap}[title={Các bước trong giai đoạn hiểu dữ liệu}]
\begin{itemize}
...
Có thể hiểu bằng \textbf{thống kê} hoặc trực quan hoá. Dưới đây là các recipe Python; dataset dùng \textbf{sklearn}.
\end{ynghia}
...
print(data.describe())
\end{lstlisting}
\textbf{Output}: shape \texttt{(768, 9)} và bảng thống kê mô tả đầy đủ các cột (count, mean, std, min, quartiles, max).
\end{vidu}
...
print(correlations)
\end{lstlisting}
Ma trận output cho tương quan giữa mọi cặp thuộc tính.
\end{vidu}
