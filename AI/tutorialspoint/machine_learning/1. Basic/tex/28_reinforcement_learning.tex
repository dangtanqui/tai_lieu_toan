...
\begin{vidu}[title={Mô hình RL điển hình}]
...
Xác định target bài toán RL: điểm số số học từ môi trường; định nghĩa sự kiện ``tốt'' và ``xấu'' cho agent.
\end{dinhnghia}
...
\begin{ynghia}[title={Positive và negative reinforcement}]
\begin{itemize}
...
\begin{ynghia}[title={Natural Language Processing}]
