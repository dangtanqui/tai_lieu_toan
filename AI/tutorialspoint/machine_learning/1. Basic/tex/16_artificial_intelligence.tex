...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_traditional_ai.htm}.
\end{ynghia}
...
\begin{vidu}[title={Tổng quan quan hệ AI--ML (theo nguồn)}]
...
\subsection{Học máy (ML) là gì?}

\begin{dinhnghia}[title={Định nghĩa ML (nhắc lại ngắn)}]
Học máy là \textbf{tập con} của AI, tập trung dạy máy học từ dữ liệu: máy tính tự học pattern và quan hệ trong dữ liệu mà không cần lập trình tường minh từng quy tắc.
Thuật toán ML phát hiện và học từ pattern để dự đoán hoặc ra quyết định.
\end{dinhnghia}

\begin{luuy}[title={Đối chiếu}]
Supervised/unsupervised/RL: Mục~\ref{sec:bon-kieu-hoc}; chi tiết từng kiểu: Mục~\ref{sec:mo-hinh-ml} trở đi.
\end{luuy}

\begin{vidu}[title={Ví dụ về ML}]
...
\begin{ynghia}[title={Bốn điểm khác biệt chính (theo nguồn)}]
\begin{enumerate}
...
\begin{vidu}[title={Khác biệt quan trọng (dịch từ bảng nguồn)}]
