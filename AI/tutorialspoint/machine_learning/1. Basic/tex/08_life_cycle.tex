...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_life_cycle.htm}.
\end{ynghia}
...
Nguồn nhấn mạnh: muốn dự án ML thành công, cần theo các bước/pha được định nghĩa rõ ràng.
\end{dongco}
...
\subsection{Các pha chính trong vòng đời (theo nguồn)}
...
Pha đầu tiên là xác định bài toán cần giải. Đây là bước quan trọng giúp bạn bắt đầu xây một solution ML.
...
Pha này gồm: hiểu bài toán kinh doanh, định nghĩa problem statement, và xác định success criteria cho mô hình.
\end{luuy}
...
Trong đó:
\begin{itemize}
  \item Data exploration: trực quan hóa và hiểu dữ liệu.
  \item Feature engineering: tạo feature mới từ feature sẵn có.
  \item Feature selection: chọn feature liên quan nhất để train mô hình.
\end{itemize}
\end{dinhnghia}

\begin{ynghia}[title={Các bước trong data preparation (theo nguồn)}]
...
Nguồn nêu một số tiêu chí khi thu thập:
\begin{itemize}
  \item \textbf{Relevant and usefulness}: dữ liệu liên quan problem statement và hữu ích để train hiệu quả.
  \item \textbf{Quality and Quantity}: chất lượng và số lượng ảnh hưởng trực tiếp đến hiệu năng.
  \item \textbf{Variety}: dữ liệu đa dạng để mô hình học nhiều kịch bản và nhận ra pattern.
\end{itemize}
...
Nguồn nhắc các công cụ như Power BI, Tableau giúp trực quan hóa để hiểu pattern/trend, từ đó hỗ trợ quyết định trong feature engineering và model selection.
\end{phuongphap}

\subsubsection{4) Feature Engineering and Selection}

\begin{dinhnghia}[title={Feature Engineering}]
...
\begin{dinhnghia}[title={Feature Selection}]
Feature selection là chọn các feature ổn định và liên quan nhất với problem statement.
Nguồn nhấn mạnh: feature engineering/selection giúp giảm kích thước dữ liệu, quan trọng khi dữ liệu tăng trưởng.
\end{dinhnghia}

\subsection{Model Development}

\begin{dongco}[title={Xây mô hình từ dữ liệu đã chuẩn bị}]
Trong pha model development, ta xây mô hình ML từ dữ liệu đã chuẩn bị.
Quy trình gồm: chọn thuật toán phù hợp, huấn luyện, tuning siêu tham số, và đánh giá bằng cross-validation.
\end{dongco}

\begin{phuongphap}[title={Ba bước chính (theo nguồn)}]
\begin{enumerate}
...
Nguồn nêu hai yếu tố cần kiểm tra:
\begin{itemize}
...
Nguồn nêu: đôi khi vấn đề không giải được chỉ bằng retrain, và chính vấn đề đó trở thành problem statement mới.
Khi đó vòng đời được “làm lại” từ khâu phân tích bài toán để phát triển mô hình cải tiến.
\end{luuy}

\begin{ynghia}[title={Kết luận của nguồn}]
...
\InputIfFileExists{_figures/03_vong_doi_du_an_ml.tex}{}{}
