...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_python_libraries.htm}.
\end{ynghia}
...
\subsection{Danh sách thư viện ML phổ biến (theo nguồn)}
...
\begin{phuongphap}[title={Các phép toán quan trọng (theo nguồn)}]
\begin{itemize}
...
\begin{vidu}[title={Ví dụ: tạo array 1 chiều (theo nguồn)}]
\begin{lstlisting}
import numpy as np
...
\begin{phuongphap}[title={5 bước xử lý dữ liệu (theo nguồn)}]
Load, Prepare, Manipulate, Model, Analyze.
\end{phuongphap}

\begin{dinhnghia}[title={Các cấu trúc dữ liệu trong Pandas (theo nguồn)}]
\begin{itemize}
  ...
  \item \textbf{Panel}: cấu trúc 3 chiều (nguồn nêu như container của DataFrame) --- \textbf{đã lỗi thời/deprecated} trong pandas; hiện dùng MultiIndex hoặc xarray.
\end{itemize}
\end{dinhnghia}

\begin{vidu}[title={Ví dụ: Series từ ndarray (theo nguồn)}]
\begin{lstlisting}
import pandas as pd
import numpy as np
...
\begin{phuongphap}[title={Cài đặt/nhập (theo nguồn)}]
\begin{itemize}
  \item Với Anaconda: thường đã có sẵn.
...
\begin{vidu}[title={Ví dụ: nghịch đảo ma trận (theo nguồn)}]
\begin{lstlisting}
import numpy as np
import scipy
...
\begin{phuongphap}[title={Đặc điểm nổi bật (theo nguồn)}]
\begin{itemize}
...
\begin{vidu}[title={Ví dụ: load Breast Cancer dataset (theo nguồn)}]
\begin{lstlisting}
...
\begin{vidu}[title={Ví dụ: NumPy array $\rightarrow$ torch tensor (theo nguồn)}]
\begin{lstlisting}
import numpy as np
import torch
x = np.ones([3,4])
...
\begin{vidu}[title={Ví dụ: tạo tensor (theo nguồn)}]
\begin{lstlisting}
...
Nó chạy trên TensorFlow/CNTK/Theano (theo nguồn) và có API trực quan, phù hợp người mới và nghiên cứu vì dễ prototype nhanh.
\end{dinhnghia}
...
\begin{vidu}[title={Ví dụ: dùng CIFAR-10 dataset (theo nguồn)}]
\begin{lstlisting}
import keras
...
\begin{vidu}[title={Ví dụ: vẽ đường thẳng (theo nguồn)}]
\begin{lstlisting}
...
Nguồn nêu tokenization và POS tagging là hai tác vụ thư viện làm tốt.
\end{dinhnghia}

\begin{luuy}[title={Các tool/framework khác}]
Nguồn nhắc thêm XGBoost, LightGBM, Gensim là các công cụ khác trong Python cho ML.
