...
\tsurl{https://www.tutorialspoint.com/machine_learning/machine_learning_data_preparation.htm}.
\end{ynghia}
...
\begin{vidu}[title={Sơ đồ quy trình chuẩn bị dữ liệu (theo nguồn)}]
...
\begin{phuongphap}[title={Chuỗi bước mục tiêu}]
...
Mục tiêu: dữ liệu chính xác, đầy đủ, liên quan cho phân tích và mô hình hoá.
\end{phuongphap}
...
Thu thập dataset từ nguồn công khai đã trình bày ở bài \textbf{Getting Datasets}; mã hoá categorical ở bài \textbf{Categorical Data}.
\end{luuy}

\begin{luuy}[title={Giảng viên lưu ý}]
Nguồn gốc dùng đường dẫn \texttt{C:\textbackslash...}; ví dụ dưới đây dùng \texttt{fetch\_openml("diabetes")} từ scikit-learn. Chi tiết train/validation/test: Mục~\ref{sec:train-val-test}.
\end{luuy}
...
\begin{phuongphap}[title={Các bước làm sạch (theo nguồn)}]
\begin{enumerate}
...
\subsubsection{1. Scaling (Min-Max)}
...
print ("\nScaled data:\n", data_rescaled[0:10])
\end{lstlisting}
\textbf{Output} (10 hàng đầu, theo nguồn): mọi giá trị nằm trong khoảng $[0,1]$.
\end{vidu}

\subsubsection{2. Normalization (L1 và L2)}
...
\subsubsection{3. Standardization}
...
\subsubsection{5. Encoding (Label Encoding)}
...
\subsubsection{6. Log Transformation}
...
\textbf{Dimensionality reduction} (giảm chiều không mất thông tin quan trọng); \textbf{Discretization} (rời rạc hoá biến liên tục như thời gian, nhiệt độ).
\end{phuongphap}
...
\begin{luuy}[title={Giảng viên lưu ý}]
Tỉ lệ train/validation/test và ý nghĩa overfitting: Mục~\ref{sec:train-val-test}.
\end{luuy}
