...
\begin{dinhnghia}
  \textbf{Mệnh đề} là một khẳng định có giá trị chân lý xác định: \textbf{đúng} (1, true) hoặc \textbf{sai} (0, false). 
...
\begin{dinhnghia}
  Cho họ tập hợp $\{A_i\}_{i \in I}$ (chỉ số trong tập $I$):
  \begin{itemize}
...
%% ================================================================
%%  CHƯƠNG 2: QUAN HỆ HAI NGÔI
%% ================================================================
\section{Quan hệ hai ngôi}

\begin{dinhnghia}
  \textbf{Quan hệ hai ngôi} $\mathcal{R}$ trên tập $X$ là một tập con của $X \times X$. Viết $x \mathcal{R} y$ (hoặc $x \sim y$) thay cho $(x, y) \in \mathcal{R}$.
\end{dinhnghia}
...
\begin{vidu}
  Trên $\mathbb{Z}$: ``$a \mid b$'' (chia hết) phản xạ, phản đối xứng, bắc cầu (thứ tự bộ phận). ``$a - b$ chẵn'': phản xạ, đối xứng, bắc cầu (tương đương).
\end{vidu}

\subsection{Quan hệ tương đương}

\begin{dinhnghia}
  Quan hệ $\sim$ trên $X$ gọi là \textbf{quan hệ tương đương} nếu có tính phản xạ, đối xứng và bắc cầu.
\end{dinhnghia}

\begin{dinhnghia}
  \textbf{Lớp tương đương} của $x$: $[x] = \{y \in X \mid y \sim x\}$. Tập thương $X/{\sim} = \{[x] \mid x \in X\}$.
\end{dinhnghia}

\begin{dinhly}
  Các lớp tương đương tạo thành một phân hoạch của $X$: mỗi phần tử thuộc đúng một lớp, hai lớp hoặc trùng nhau hoặc rời nhau.
\end{dinhly}

\begin{vidu}
  Trên $\mathbb{Z}$, quan hệ ``đồng dư modulo $n$'': $a \equiv b \pmod{n}$ $\Leftrightarrow$ $n \mid (a-b)$. Đây là quan hệ tương đương. Các lớp: $[0], [1], \ldots, [n-1]$ tạo thành $\mathbb{Z}/n\mathbb{Z}$.
\end{vidu}

\subsection{Quan hệ thứ tự}

\begin{dinhnghia}
  Quan hệ $\le$ trên $X$ gọi là \textbf{quan hệ thứ tự} (bộ phận) nếu có tính phản xạ, phản đối xứng và bắc cầu. Khi đó $(X, \le)$ gọi là \textbf{tập sắp thứ tự}.
\end{dinhnghia}

\begin{dinhnghia}
  Thứ tự \textbf{toàn phần} (tuyến tính): mọi cặp phần tử đều so sánh được ($\forall x, y$: $x \le y$ hoặc $y \le x$).
\end{dinhnghia}

\begin{vidu}
  $(\mathbb{N}, \le)$, $(\mathbb{R}, \le)$: thứ tự toàn phần. $(\mathcal{P}(X), \subset)$: thứ tự bộ phận (không toàn phần nếu $X$ có hơn 1 phần tử: $\{1\}$ và $\{2\}$ không so sánh được).
\end{vidu}
...
\begin{vidu}
  $f: \mathbb{R} \to \mathbb{R}$, $f(x) = x^2$. $f(\{-1, 1\}) = \{1\}$. $f^{-1}(\{4\}) = \{-2, 2\}$. $f^{-1}([-1, 4]) = [-2, 2]$.
\end{vidu}
...
\begin{tinhchat}
  Nếu $f: A \to B$ đơn ánh thì $f: A \to \mathrm{Im}(f)$ là toàn ánh. Mọi song ánh đều có thể xem như đơn ánh từ $A$ lên $B$.
\end{tinhchat}
...
\begin{dinhnghia}
  Cho $A \subset X$. \textbf{Ánh xạ đặc trưng} (hàm chỉ tiêu) $\chi_A: X \to \{0, 1\}$: $\chi_A(x) = 1$ nếu $x \in A$, $\chi_A(x) = 0$ nếu $x \notin A$.
\end{dinhnghia}

\begin{dinhnghia}
  Cho $A_1 \times A_2 \times \cdots \times A_n$. \textbf{Phép chiếu} thứ $i$: $\pi_i: A_1 \times \cdots \times A_n \to A_i$, $\pi_i(a_1, \ldots, a_n) = a_i$.
\end{dinhnghia}
...
\begin{vidu}
  $f: \mathbb{R} \to \mathbb{R}$, $f(x) = 5$ là ánh xạ hằng. $\pi_1: \mathbb{R}^2 \to \mathbb{R}$, $\pi_1(x,y) = x$ là phép chiếu lên trục hoành.
\end{vidu}
...
\begin{dinhnghia}
  \textbf{Hợp ánh xạ}: $(g \circ f)(x) = g(f(x))$. Điều kiện: $\mathrm{Im}(f) \subset$ miền xác định của $g$.
\end{dinhnghia}
...
\begin{vidu}
  $f: \mathbb{R} \to \mathbb{R}^+$, $f(x) = e^x$ (song ánh). $g: \mathbb{R}^+ \to \mathbb{R}$, $g(y) = \ln y$ (song ánh). Khi đó $g \circ f = \mathrm{id}_\mathbb{R}$, $f \circ g = \mathrm{id}_{\mathbb{R}^+}$.
\end{vidu}
...
\begin{dinhnghia}
  $g: D \to B$ gọi là \textbf{mở rộng} của $f: A \to B$ nếu $A \subset D$ và $g|_A = f$.
\end{dinhnghia}
...
\begin{dinhnghia}
  Tập $A$ \textbf{hữu hạn} nếu $A = \varnothing$ hoặc $A \sim \{1, 2, \ldots, n\}$ với $n \in \mathbb{N}$. Khi đó $|A| = n$. Tập \textbf{vô hạn}: không hữu hạn.
\end{dinhnghia}

\begin{dinhly}
  $\mathbb{N}$, $\mathbb{Z}$, $\mathbb{Q}$ đều đếm được (đồng lực lượng với $\mathbb{N}$). $\mathbb{R}$ không đếm được (lực lượng lớn hơn $\mathbb{N}$).
\end{dinhly}
...
\begin{dinhnghia}
  \textbf{Số phức} $z = a + bi$, với $a, b \in \mathbb{R}$, $i^2 = -1$.
  \begin{itemize}
    \item $a = \mathrm{Re}(z)$: phần thực.
    \item $b = \mathrm{Im}(z)$: phần ảo.
    \item Số phức liên hợp: $\overline{z} = a - bi$.
  \end{itemize}
\end{dinhnghia}
...
\begin{vidu}
  $P(z) = z^2 + 1$ có nghiệm $i$, $-i$. $P(z) = z^n - 1$ có $n$ nghiệm là các căn bậc $n$ của đơn vị: $e^{2\pi ik/n}$, $k = 0, \ldots, n-1$.
\end{vidu}
...
\begin{dinhnghia}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Phần tử trung hòa} $e$: $a * e = e * a = a$ với mọi $a \in G$.
    \item \textbf{Phần tử đối xứng}: $\forall a \in G$, $\exists a' \in G$: $a * a' = a' * a = e$.
  \end{enumerate}
  Nếu thêm \textbf{giao hoán} $a * b = b * a$ thì gọi là \textbf{nhóm Abel} (giao hoán).
\end{dinhnghia}
...
\begin{tinhchat}[title={Tính chất của nhóm}]
  \begin{itemize}
    ...
    \item Luật giản ước: $a * x = a * y \Rightarrow x = y$.
...
\begin{dinhnghia}
  ...
  \begin{itemize}
    \item $(R, +)$ là nhóm Abel.
    \item Phép nhân $\cdot$ kết hợp.
    \item Phân phối: $a \cdot (b + c) = a \cdot b + a \cdot c$, $(a + b) \cdot c = a \cdot c + b \cdot c$.
  \end{itemize}
  Vành \textbf{giao hoán} nếu $a \cdot b = b \cdot a$. Vành \textbf{có đơn vị} nếu có phần tử $1$: $1 \cdot a = a \cdot 1 = a$.
\end{dinhnghia}

\begin{vidu}
  $(\mathbb{Z}, +, \cdot)$, $(\mathbb{Q}, +, \cdot)$, $(\mathbb{R}, +, \cdot)$: vành giao hoán có đơn vị. Tập các ma trận vuông cấp $n$: vành có đơn vị nhưng không giao hoán khi $n > 1$.
\end{vidu}

\subsection{Trường}

\begin{dinhnghia}
  \textbf{Trường} $K$: vành giao hoán có đơn vị sao cho mọi phần tử khác 0 đều có nghịch đảo: $\forall a \in K$, $a \ne 0$, $\exists a^{-1}$: $a \cdot a^{-1} = 1$.
\end{dinhnghia}

\begin{tinhchat}[title={Tính chất của trường}]
  \begin{itemize}
    \item Trường là vành nguyên (không có ước của 0).
    \item $K^* = K \setminus \{0\}$ là nhóm với phép nhân.
    \item Luật giản ước: $a \ne 0$, $a \cdot x = a \cdot y \Rightarrow x = y$.
