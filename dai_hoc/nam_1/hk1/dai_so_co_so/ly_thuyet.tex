...
\begin{luuy}
  Ký hiệu $:=$: định nghĩa bởi

  Có thể biểu diễn tập hợp qua biểu đồ \textbf{Ven}
\end{luuy}
...
\begin{vidu}
  ...
  \item ``$a - b$ chẵn'': phản xạ, đối xứng, bắc cầu. Không phản đối xứng, vì với $a - b \text{ chẳn và } b - a \text{ chẳn}$ chưa chắc $a = b$
  \item $a > b$: bắc cầu. Không phản xạ (vì $a > a$ là sai), không đối xứng (vì $b > a$ là sai), không phản đối xứng (vì với $a > b$ và ${b > a}$ thì vô lý
  \end{itemize}
\end{vidu}
...
\begin{dinhnghia}
  Quan hệ $\mathcal{R}$ trên $X$ gọi là \textbf{quan hệ tương đương} nếu có tính phản xạ, đối xứng và bắc cầu. Ký hiệu: $A \sim B \text{ } (\mathcal{R})$
\end{dinhnghia}
...
\begin{dinhnghia}
  Thứ tự \textbf{toàn phần} (tuyến tính): Một quan hệ $\mathcal{R}$ có thứ tự trên tập $X$ và $\forall a, b \in X$ ta đều có $a \mathcal{R} b$ và $b \mathcal{R} a$
\end{dinhnghia}
...
\begin{dinhnghia}
  \textbf{Ánh xạ đồng nhất} trên $A$: $\mathrm{id}_A: A \to A$
  
  $\mathrm{id}_A(x) = x$ với mọi $x \in A$.
\end{dinhnghia}
...
\begin{dinhnghia}
  ...
  \textbf{Hợp ánh xạ (tích ánh xạ)}: $(g \circ f)(x) = g(f(x))$. 
...
\begin{dinhnghia}
  Hai tập $A$ và $B$ gọi là \textbf{đồng lực lượng} (cùng lực lượng) nếu tồn tại song ánh $f: A \to B$. Ký hiệu $|A| = |B|$ hoặc $A \mathcal{R} B$.
\end{dinhnghia}

\begin{dinhnghia}
  Tập $A$ \textbf{hữu hạn} nếu $A = \varnothing$ hoặc $A \mathcal{R} \{1, 2, \ldots, n\}$ với $n \in \mathbb{N}$. Khi đó $|A| = n$. 
...
\begin{dinhnghia}
  ...
  \begin{itemize}
    ...
    \item Luật nhân ($\cdot$) có tính phân phối 2 phía với luật cộng ($+$): 
    \begin{itemize}
        \item Phân phối trái: $a \cdot (b + c) = a \cdot b + a \cdot c$.
        \item Phân phối phải: $(a + b) \cdot c = a \cdot c + b \cdot c$.
    \end{itemize}
  \end{itemize}
...
\begin{dinhnghia}
  \textbf{Vành nguyên} là 1 vành $(G, +, \cdot)$ trong đó có tinh chất: $a.b = 0 \Rightarrow a = 0 \text{ hoặc } b = 0$
\end{dinhnghia}

\subsection{Trường}

\begin{dinhnghia}
  \textbf{Trường} $(K, +, \cdot)$: : tập R với hai phép toán + và · thỏa:
  \begin{itemize}
...
\begin{dinhnghia}
  \textbf{Số phức} (dạng chính tắt) $z = a + bi$, với $a, b \in \mathbb{R}$, $i^2 = -1$.
  \begin{itemize}
...
Tập $\mathbb{C}$ là 1 trường
- Phần tử trung hòa của luật cộng ($+$) là $(0, 0)$
- Phần tử đối của luật cộng ($+$) với $(a, b)$ là $(-a, -b) = -(a, b)$ gọi là số phức đối
- Phần tử trung hòa của luật nhân ($\cdot$) là $(1, 0)$
- Phần tử đối của luật nhân ($\cdot$) với $(a, b) \ne (0, 0)$ là $(\dfrac{a}{a^2 + b^2}, \dfrac{-b}{a^2 + b^2}) = (a, b)^-1$ là số phức nghịch đảo

Mặt phẳng phức $Oxy$ với tọa độ $M = (a, b)$, số phức $z = a + bi$ gọi là \textbf{tọa vị} của M.

Ảnh của số phức (a, 0), trục thực $Ox$.

Số thực là trường hợp riêng của số phức

Số ảo thuần túy (0, b) có bình phương luôn là số âm ($b \ne 0$). Đơn vị ảo $(0, 1)$, trục ảo $Oy$
...
\begin{congthuc}[title={Tổng}]
  $z_1 + z_2 = a + a' + (b + b')i$. Điểm $A(a, b)$, $B(a', b')$, $C(a + a', b + b')$ tạo ra 3 vector có phép cộng theo quy tắt hình bình nhành.

  Khi đó: $|z1 + z_2| \le |z_1| + |z_2|$
\end{congthuc}
...
\subsection{Giải phương trình bật 2}
