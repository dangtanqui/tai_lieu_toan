  ...
  \begin{itemize}
    ...
    \item \textbf{Đổi đơn vị:}
      \[
        ...
        1\;\text{rad} = \dfrac{180^\circ}{\pi},\qquad
...
\begin{congthuc}[title={Các góc liên quan đặc biệt}]
  \begin{itemize}
    ...
    \item \textbf{Phụ nhau} (phụ -- chéo):
      \[
        \sin\!\left(\dfrac{\pi}{2} - \alpha\right) = \cos\alpha,\quad
        \cos\!\left(\dfrac{\pi}{2} - \alpha\right) = \sin\alpha,\quad
...
\begin{congthuc}[title={Công thức cộng}]
  \begin{align*}
    ...
    \tan(\alpha + \beta) &= \dfrac{\tan\alpha + \tan\beta}{1 - \tan\alpha\cdot\tan\beta}, \\[4pt]
...
\begin{congthuc}[title={Công thức nhân ba}]
  \begin{align*}
...
\begin{congthuc}[title={Công thức hạ bậc}]
  \[
    \sin^2\alpha = \dfrac{1 - \cos 2\alpha}{2},\qquad
...
\begin{congthuc}
  \begin{itemize}
    ...
    \item Tổng $n$ số hạng đầu:
      \[
        S_n = n\,\dfrac{u_1 + u_n}{2}
            = n\,u_1 + \dfrac{n(n-1)}{2}\,d.
      \]
  \end{itemize}
\end{congthuc}
...
\begin{dinhnghia}
  ...
  \[
    \dfrac{u_{n+1}}{u_n} = q,\quad \forall n \ge 1
  \]
...
\begin{congthuc}
  \begin{itemize}
    ...
    \item Tổng $n$ số hạng đầu ($q \ne 1$):
      \[
        S_n = u_1 \cdot \dfrac{1 - q^n}{1 - q} = u_1 \cdot \dfrac{q^n - 1}{q - 1}.
      \]
...
\begin{congthuc}[title={Cấp số nhân lùi vô hạn}]
  ...
  \[
    S = \lim S_n = \dfrac{u_1}{1 - q}.
  \]
\end{congthuc}

\begin{vidu}
  $\displaystyle\lim \dfrac{3n^2 + 1}{n^2 - 2}
...
\begin{tinhchat}[title={Quy tắc tính}]
  ...
  \begin{itemize}
    ...
    \item $\displaystyle\lim_{x \to a}\dfrac{f(x)}{g(x)} = \dfrac{L}{M}$ \;(khi $M \ne 0$).
  \end{itemize}
...
\begin{dinhnghia}
  \begin{itemize}
    ...
    \item $a^{1/n} = \sqrt[n]{a}$. \quad $a^{m/n} = \sqrt[n]{a^m}$ \;($a > 0$, $n \in \mathbb{N}^*$, $n \ge 2$).
  \end{itemize}
\end{dinhnghia}
...
\begin{congthuc}[title={Tính chất logarit}]
  ...
  \begin{align*}
    ...
    \log_a(x^n) &= n\,\log_a x. &
    \log_{a^{m}}(x^n) &= \dfrac{n}{m}\,\log_a x.
  \end{align*}
  \textbf{Đổi cơ số:}
  \[
    \log_a b = \dfrac{\log_c b}{\log_c a},\qquad \log_c a\log_a b = \log_c b,\qquad
...
\begin{congthuc}[title={Bảng đạo hàm cơ bản}]
  \begin{align*}
    ...
    \left(\dfrac{1}{x^n}\right)' &= -\dfrac{n}{x^{n + 1}}, \\[4pt]
    (\sin x)' &= \cos x, &
    (\cos x)' &= -\sin x, \\[2pt]
    (\tan x)' &= \dfrac{1}{\cos^2 x}, &
    (\cot x)' &= -\dfrac{1}{\sin^2 x}, \\[2pt]
    (\dfrac{ax + b}{cx + d})' &= \dfrac{ad - bc}{(cx + d)^2}, & &
    (\dfrac{ax^2 + bx + c}{dx + e})' &= \dfrac{adx^2 + 2aex + be - cd}{(dx + e)^2}.
  \end{align*}
\end{congthuc}
...
\begin{congthuc}[title={Đạo hàm hàm hợp}]
  ...
  \begin{align*}
    ...
    \left(\dfrac{1}{u}\right)' &= -\dfrac{u'}{u^2}, \\[4pt]
    (\sin u)' &= u'\cos u, &
    (\cos u)' &= -u'\sin u, \\[2pt]
    (\tan u)' &= \dfrac{u'}{\cos^2 u}, &
    (\cot u)' &= -\dfrac{u'}{\sin^2 u}, \\[2pt]
    (e^u)' &= u'e^u, &
    (a^u)' &= u'a^u\,\ln a, \\[2pt]
    (\ln u)' &= \dfrac{u'}{u}, &
...
\begin{congthuc}[title={Số trung bình}]
  \[
    \bar{x} = \dfrac{n_1\,c_1 + n_2\,c_2 + \cdots + n_k\,c_k}{n},
    \qquad c_i = \dfrac{a_i + a_{i+1}}{2} \;\text{(giá trị đại diện nhóm)}.
  \]
\end{congthuc}
...
\begin{congthuc}[title={Trung vị $M_e$}]
  \begin{enumerate}
    ...
    \item Công thức:
      \[
        M_e = a_p + \dfrac{\dfrac{n}{2} - F_{p-1}}{n_p}\,(a_{p+1} - a_p),
      \]
...
\begin{congthuc}[title={Tứ phân vị}]
  \begin{itemize}
    \item \textbf{$Q_1$:} tìm nhóm chứa $Q_1$ (tần số tích lũy vượt $\dfrac{n}{4}$):
      \[
        Q_1 = a_p + \dfrac{\dfrac{n}{4} - F_{p-1}}{n_p}\,(a_{p+1} - a_p).
      \]
    \item \textbf{$Q_2 = M_e$} (trung vị).
    \item \textbf{$Q_3$:} tìm nhóm chứa $Q_3$ (tần số tích lũy vượt $\dfrac{3n}{4}$):
      \[
        Q_3 = a_p + \dfrac{\dfrac{3n}{4} - F_{p-1}}{n_p}\,(a_{p+1} - a_p).
      \]
  \end{itemize}
\end{congthuc}
...
\begin{congthuc}[title={Mốt $M_o$}]
  \begin{enumerate}
    ...
    \item Công thức:
      \[
        M_o = a_j + \dfrac{n_j - n_{j-1}}{(n_j - n_{j-1}) + (n_j - n_{j+1})} \cdot h,
      \]
...
\begin{dinhly}[title={Các tính chất thừa nhận (tiên đề)}]
  \begin{enumerate}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_di_qua_2_diem.png}
    \end{center}
    \item Qua 3 điểm không thẳng hàng xác định duy nhất 1 mặt phẳng.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phang_di_qua_3_diem.png}
    \end{center}
    \item Tồn tại 4 điểm không cùng thuộc một mặt phẳng.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/4_diem_khong_thuoc_1_mat_phang.png}
    \end{center}
    \item Nếu đường thẳng có 2 điểm thuộc mặt phẳng thì nằm trọn trong mặt phẳng đó.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_diem_thuoc_mat_phang.png}
    \end{center}
    \item Hai mặt phẳng có 1 điểm chung thì có chung 1 đường thẳng (giao tuyến).
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/giao_tuyen_2_mat_phang.png}
    \end{center}
  \end{enumerate}
\end{dinhly}
...
\begin{tinhchat}[title={Cách xác định mặt phẳng}]
  \begin{enumerate}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phang_qua_1_diem_va_1_duong_thang.png}
    \end{center}
    \item Chứa 2 đường thẳng cắt nhau.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phan_chua_2_duong_thang_cat_nhau.png}
    \end{center}
  \end{enumerate}
\end{tinhchat}
...
\begin{dinhnghia}
  \begin{itemize}
    ...
    \begin{center}
        \includegraphics[width=1\linewidth]{cap_3//hinh_11/2_duong_thang_dong_phang.png}
    \end{center}
    \item Hai đường thẳng \textbf{chéo nhau}: không cùng nằm trên bất kỳ mặt phẳng nào.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_duong_thang_cheo_nhau.png}
    \end{center}
  \end{itemize}
\end{dinhnghia}
...
\begin{dinhly}
  \begin{enumerate}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/1_duong_thang_di_qua_1_diem_va_song_song_1_duong_thang.png}
    \end{center}
    \item Nếu 2 đường thẳng cùng song song với đường thẳng thứ ba thì song song với nhau.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/3_duong_thang_song_song.png}
    \end{center}
    \item Nếu 3 mặt phẳng đôi một cắt nhau thì 3 giao tuyến phân biệt đồng quy hoặc đôi một song song.
    \begin{center}
        \includegraphics[width=0.75\linewidth]{cap_3//hinh_11/3_mat_phang_cat_nhau.png}
    \end{center}
  \end{enumerate}
\end{dinhly}
...
\begin{dinhnghia}
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_song_song_mat_phang.png}
  \end{center}
\end{dinhnghia}
...
\begin{dinhly}[title={Dấu hiệu nhận biết}]
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_song_song_1_duong_thang_tren_mat_phang.png}
  \end{center}
\end{dinhly}

\begin{tinhchat}
  \begin{itemize}
    \item Nếu đường thẳng $d$ và mặt phẳng $(P)$ không có điểm chung: $d$ song song $(P)$.
    \item Nếu đường thẳng $d$ và mặt phẳng $(P)$ có đúng 1 điểm chung: $d$ cắt $(P)$.
    \item Nếu đường thẳng $d$ và mặt phẳng $(P)$ có nhiều hơn 1 điểm chung: $d \subset (P)$.
    \begin{center}
        \includegraphics[width=1\linewidth]{cap_3//hinh_11/vi_tri_giua_duong_thang_va_mat_phang.png}
    \end{center}
    \item Nếu $d \parallel (P_1)$ và $d \parallel (P_2)$ thì $d$ song song với giao tuyến của $(P_1)$ và $(P_2)$ (nếu tồn tại).
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_song_song_2_mat_phang.png}
    \end{center}
  \end{itemize}
\end{tinhchat}
...
\begin{dinhnghia}
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_mat_phang_song_song.png}
  \end{center}
\end{dinhnghia}
...
\begin{tinhchat}
  \begin{itemize}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phang_cat_2_mat_phang_song_song.png}
    \end{center}
    \item Qua 1 điểm ngoài $(P)$ có duy nhất 1 mặt phẳng song song với $(P)$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phang_qua_1_diem_va_song_song_1_mat_phang.png}
    \end{center}
    \item Hai mặt phẳng phân biệt cùng song song với mặt phẳng thứ ba thì song song với nhau.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/3_mat_phang_song_song.png}
    \end{center}
  \end{itemize}
\end{tinhchat}
...
\begin{dinhnghia}
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_vuong_goc_mat_phang.png}
  \end{center}
\end{dinhnghia}
...
\begin{dinhly}[title={Dấu hiệu nhận biết}]
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_vuong_goc_2_duong_thang_cat_nhau_tren_mat_phang.png}
  \end{center}
\end{dinhly}
...
\begin{tinhchat}
  \begin{itemize}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_di_qua_1_diem_va_vuong_goc_1_mat_phang.png}
    \end{center}
    \item Qua 1 điểm có duy nhất 1 mặt phẳng vuông góc với đường thẳng $d$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phang_di_qua_1_diem_va_vuong_goc_1_duong_thang.png}
    \end{center}
    \item \textbf{Mặt phẳng trung trực} của $AB$: mặt phẳng đi qua trung điểm của $AB$ và vuông góc với $AB$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/mat_phang_trung_truc.png}
    \end{center}
    \item Nếu $d \perp (P)$ thì mọi đường thẳng song song với $d$ đều vuông góc với $(P)$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_duong_thang_song_song_cung_vuong_goc_1_mat_phang.png}
    \end{center}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_mat_phang_vuong_goc_1_duong_thang.png}
    \end{center}
  \end{itemize}
\end{tinhchat}
...
\begin{luuy}
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_vuong_goc_tam_giac.png}
  \end{center}
\end{luuy}
...
\begin{dinhly}
  ...
  \]
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_vuong_goc_hinh_chieu_duong_thang_tren_mat_phang.png}
  \end{center}
\end{dinhly}
...
\begin{luuy}
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/goc_giua_duong_thang_va_mat_phang.png}
  \end{center}
\end{luuy}
...
\begin{dinhnghia}
  \begin{itemize}
    \item Gọi $d'$ là hình chiếu vuông góc của $d$ lên $(P)$. \textbf{Góc giữa $d$ và $(P)$:} là góc $\widehat{(d,\, d')}$; thuộc $[0^\circ;\, 90^\circ]$.
    \item $d \perp (P)$: góc $= 90^\circ$. \quad $d \parallel (P)$ hoặc $d \subset (P)$: góc $= 0^\circ$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/goc_duong_thang_mat_phang_90.png}
    \end{center}
    \begin{center}
        \includegraphics[width=1\linewidth]{cap_3//hinh_11/goc_duong_thang_mat_phang_0.png}
    \end{center}
  \end{itemize}
\end{dinhnghia}
...
\begin{dinhnghia}
  \begin{itemize}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/goc_giua_2_mat_phang.png}
    \end{center}
    \item \textbf{Hai mp vuông góc:} góc giữa chúng bằng $90^\circ$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_mat_phang_vuong_goc.png}
    \end{center}
  \end{itemize}
\end{dinhnghia}
...
\begin{dinhly}[title={Dấu hiệu nhận biết}]
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_thang_thuoc_mat_phang_vuong_goc_1_mat_phang.png}
  \end{center}
\end{dinhly}
...
\begin{tinhchat}
  \begin{itemize}
    ...
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/2_mat_phang_vuong_goc_1_mat_phang.png}
    \end{center}
  \end{itemize}
\end{tinhchat}
...
\begin{dinhnghia}
  \begin{itemize}
    \item \textbf{Khoảng cách từ điểm $M$ đến đường thẳng $\Delta$:} $d(M,\,\Delta) = MH$, với $H$ là hình chiếu vuông góc của $M$ lên $\Delta$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/khoang_cach_1_diem_den_1_duong_thang.png}
    \end{center}
    \item \textbf{Khoảng cách từ điểm $M$ đến mặt phẳng $(P)$:} $d(M,\,(P)) = MH$, với $H$ là hình chiếu vuông góc của $M$ lên $(P)$.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/khoang_cach_1_diem_den_1_mat_phang.png}
    \end{center}
    \item \textbf{Khoảng cách giữa 2 đường thẳng song song:} lấy 1 điểm trên đường thẳng này, tính khoảng cách đến đường thẳng kia.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/khoang_cach_giua_2_duong_thang_song_song.png}
    \end{center}
    \item \textbf{Khoảng cách giữa đường thẳng và mp song song:} lấy 1 điểm trên đường thẳng, tính khoảng cách đến mp.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/khoang_cach_giua_duong_thang_song_song_mat_phang.png}
    \end{center}
    \item \textbf{Khoảng cách giữa 2 mp song song:} lấy 1 điểm trên mp này, tính khoảng cách đến mp kia.
    \begin{center}
        \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/khoang_cach_giua_2_mat_phang_song_song.png}
    \end{center}
  \end{itemize}
\end{dinhnghia}
...
\begin{dinhnghia}[title={Khoảng cách giữa 2 đường chéo nhau}]
  ...
  \begin{center}
      \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/duong_vuong_goc_chung.png}
  \end{center}
\end{dinhnghia}
...
\begin{phuongphap}[title={Các cách tính khoảng cách}]
  \begin{enumerate}
    ...
    \item \textbf{Dùng thể tích:} $V = \dfrac{1}{3}\,S_{\text{đáy}} \cdot h$ $\;\Rightarrow\;$ $h = \dfrac{3V}{S_{\text{đáy}}}$.
    \item \textbf{Dùng mặt phẳng trung gian:} Khoảng cách giữa 2 đường chéo nhau $d_1$, $d_2$: dựng $(P) \parallel d_2$ chứa $d_1$, thì $d(d_1,\,d_2) = d(d_2,\,(P))$.
      \begin{center}
          \includegraphics[width=0.5\linewidth]{cap_3//hinh_11/dung_mat_phang_chua_duong_thang_nay_va_song_song_duong_thang_kia.png}
      \end{center}
  \end{enumerate}
\end{phuongphap}
...
\subsection{Thể tích khối đa diện}

\begin{congthuc}[title={Thể tích các khối đa diện}]
  \begin{itemize}
    \item \textbf{Khối hộp chữ nhật} (kích thước $a$, $b$, $c$):
      \[V = a \cdot b \cdot c\]
    \item \textbf{Khối lập phương} (cạnh $a$):
      \[V = a^3\]
    \item \textbf{Khối lăng trụ} (diện tích đáy $S$, chiều cao $h$):
      \[V = S \cdot h\]
    \item \textbf{Khối chóp} (diện tích đáy $S$, chiều cao $h$):
      \[V = \dfrac{1}{3}\,S \cdot h\]
    \item \textbf{Khối chóp cụt} (diện tích đáy lớn $S_1$, đáy nhỏ $S_2$, chiều cao $h$):
      \[V = \dfrac{h}{3}\bigl(S_1 + S_2 + \sqrt{S_1 \cdot S_2}\bigr)\]
  \end{itemize}
\end{congthuc}

\begin{tinhchat}
  \begin{itemize}
    \item Nếu mặt phẳng $(P)$ chia khối chóp thành hai phần, tỉ số thể tích phụ thuộc vào tỉ số diện tích và chiều cao tương ứng.
    \item \textbf{Tỉ số thể tích:} Hai khối chóp chung đỉnh, đáy cùng nằm trên một mặt phẳng thì tỉ số thể tích bằng tỉ số diện tích đáy.
  \end{itemize}
\end{tinhchat}

\begin{congthuc}[title={Diện tích xung quanh hình chóp đều}]
  \[
    S_{xq} = \dfrac{1}{2}\,p \cdot d
  \]
...
\medskip\noindent\textbf{Các dạng bài tập:}
\begin{enumerate}[label=\textbf{Dạng \arabic*:}, leftmargin=*, align=left]
  \item Tính thể tích khối lăng trụ.
  \item Tính thể tích khối chóp.
  \item Tính tỉ số thể tích.
