...
\begin{itemize}
  ...
  \item \textbf{Bậc của đơn thức} (đã thu gọn, khác $0$) là tổng số mũ của tất cả các biến.
  \item \textbf{Đa thức} là tổng của các đơn thức.
  \item \textbf{Bậc của đa thức} là bậc cao nhất trong các đơn thức (đã thu gọn) của đa thức.
  \item \textbf{Đa thức thu gọn:} đa thức không chứa hai hạng tử đồng dạng.
  \item \textbf{Đa thức một biến} sắp xếp theo lũy thừa giảm hoặc tăng của biến.
\end{itemize}
\end{dinhnghia}
...
\begin{dinhnghia}[title={Nghiệm của đa thức một biến}]
Số $a$ được gọi là \textbf{nghiệm} của đa thức $P(x)$ nếu $P(a) = 0$.
\end{dinhnghia}

\begin{vidu}
Đa thức $P(x) = x^2 - 4$ có nghiệm $x = 2$ và $x = -2$ vì $P(2) = 4 - 4 = 0$ và $P(-2) = 4 - 4 = 0$.
\end{vidu}

\begin{luuy}
Một đa thức bậc $n$ (khác đa thức không) có không quá $n$ nghiệm.
\end{luuy}
...
\medskip
\textbf{Diện tích toàn phần} (cho mọi hình khối):
\[
