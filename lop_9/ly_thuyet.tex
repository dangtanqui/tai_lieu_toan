...
\begin{dinhnghia}
  \begin{itemize}
    \item \textbf{Căn bậc hai số học} của $a \ge 0$ là số $x \ge 0$ sao cho $x^2 = a$, kí hiệu $x = \sqrt{a}$.
    \item Số dương $a$ có hai căn bậc hai: $\sqrt{a}$ (dương) và $-\sqrt{a}$ (âm).
    \item $\sqrt{a}$ có nghĩa (xác định) khi và chỉ khi $a \ge 0$.
    \item $\sqrt{a^2} = |a|$, \quad $\bigl(\sqrt{a}\bigr)^2 = a$ \;($a \ge 0$).
  \end{itemize}
\end{dinhnghia}
...
\begin{phuongphap}[title={Ứng dụng định lý Vi-ét}]
  \begin{itemize}
    \item Tìm hai số khi biết tổng $S$ và tích $P$: hai số đó là nghiệm của $t^2 - St + P = 0$.
    \item Tính các biểu thức đối xứng của hai nghiệm:
      \begin{itemize}
        \item $x_1^2 + x_2^2 = S^2 - 2P$
        \item $|x_1 - x_2| = \sqrt{S^2 - 4P}$
        \item $x_1^3 + x_2^3 = S^3 - 3SP$
      \end{itemize}
    \item Xét dấu hai nghiệm dựa trên $S$ và $P$.
  \end{itemize}
\end{phuongphap}
...
\begin{dinhly}[title={Liên hệ giữa đường kính và dây cung}]
  \begin{itemize}
    \item Đường kính vuông góc với một dây thì đi qua trung điểm của dây đó.
    \item Đường kính đi qua trung điểm của một dây (không phải đường kính) thì vuông góc với dây đó.
  \end{itemize}
\end{dinhly}
...
\subsubsection{Hình nón cụt}

\begin{congthuc}
  Hình nón cụt có bán kính đáy lớn $R$, bán kính đáy nhỏ $r$, chiều cao $h$, đường sinh $l$:
  \begin{itemize}
    \item Diện tích xung quanh: $S_{xq} = \pi(R + r) l$
    \item Thể tích: $V = \dfrac{1}{3}\pi h(R^2 + Rr + r^2)$
  \end{itemize}
\end{congthuc}
